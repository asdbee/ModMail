...
module.exports = {
    ...
    execute(bot,msg,args,checkMail){
        ...
        mail.getModMail(user).then((mm) => {
            ...
            const userObject = bot.guilds.get(config.mainGuild).members.get(user)
            if (userObject === undefined) return bot.createMessage(msg.channel.id,'`!` This user isn\'t in this server!')
